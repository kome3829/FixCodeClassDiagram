...
void Enemy::action()
{
    if (mIsActive)
    {
        ...
        if (!mIsBossStage)
        {
            if (mMoveCount % MOVE_INTERVAL_FRAME == 0 && mIsActive == true)//２フレームに一回移動
            {
                //y100まで降下
				...
				//画面外処理
                if (mX >= MAX_SCREEN_WIDTH + mWidth || mX <= 0 - mWidth || mY >= MAX_SCREEN_HEIGHT + mHeight || mY <= 0 - mHeight)
                {
                    mIsActive = false;
                    mIsDefeat = true;
                }
        ...
        else
        {
            if (mNumber == SUMMON_LEFT)//左側
            {
                ...
                if (abs((int)mX - ENEMY_SUMMON_POINT_LEFT_X) >= ENEMY_POSITION_DIFFERENCE_RANGE && abs((int)mY - ENEMY_SUMMON_POINT_LEFT_Y) >= ENEMY_POSITION_DIFFERENCE_RANGE &&!mIsReachedBossPosition)
                {
            ...
            if (mNumber == SUMMON_RIGHT)//右側
            {
				//位置についたか判定
				//現在の位置と指定位置のずれが一定値になるまで処理を実行する
                if (abs((int)mX - ENEMY_SUMMON_POINT_RIGHT_X) >= ENEMY_POSITION_DIFFERENCE_RANGE &&abs((int)mY - ENEMY_SUMMON_POINT_RIGHT_Y) >= ENEMY_POSITION_DIFFERENCE_RANGE &&!mIsReachedBossPosition)
                {
					//atan2を使用して敵と指定位置とのなす角を計算
...
@note
  ...
  - フラグ（FLG）が true の場合のみ処理を実行する
  - ダメージフラグ（dmgFlg）が true の場合は、一定時間ダメージ用の敵画像を表示する
  - なにもなければは通常の敵画像を表示する
...
void Enemy::draw()
{
	if (mIsActive)
	{
		...
		{
			//通常画像の表示
			DrawGraph((int)mX - mWidth / CUT_HALF, (int)mY - mHeight / CUT_HALF, Data::getInstance()->mEnemyImageHandle, TRUE);
		}
	}
}
...
@note     フラグ（FLG）を true にすることで、action 関数および draw 関数の処理対象となる
@note     引数で位置、敵の番号、ボスステージかどうかを指定できる
@note     ボスステージフラグ（BOSS_STAGE）に応じて、位置・番号・体力などのパラメータ設定処理を切り替える
@warning
- 本関数は、継承先の敵クラスでも共通の出現処理として使用される
*/
void Enemy::pop(int popPositionX, int popPositionY, int number, bool isBossStage)
{
	if(!isBossStage)
	{ 
		//パラメータの設定
...
@param[in]	MissileBullet* 	missileBullet		:ミサイル弾クラスのインスタンスポインタ
@param[in]	SpecialBullet* 	specialBullet		:スペシャル弾クラスのインスタンスポインタ
@param[in]	HomingBullet* 	homingBullet		:ホーミング弾クラスのインスタンスポインタ
...
@note     
  ...
  - フラグ（FLG）および各プレイヤー弾のフラグ（flg）が true の場合のみ処理を実行する
...
*/
bool Enemy::checkPlayerBulletHit(Bullet* bullet, MissileBullet* missileBullet, SpecialBullet* specialBullet, HomingBullet* homingBullet, int* score)
{
	//通常弾の判定
	if (mIsActive && bullet->mIsActive)
	{
		//敵と通常弾の位置を計算して弾が一定範囲内なら当たり判定を返す
		if (abs((int)(bullet->mX - mX)) < (mWidth / CUT_HALF + HITBOX_MARGIN_X) && abs((int)(bullet->mY - mY)) < mHeight / CUT_HALF + HITBOX_MARGIN_Y)
		{
			//弾の無効化
	...
	if (mIsActive && missileBullet->mIsActive)
	{
		...
		if (abs((int)(missileBullet->mX - mX)) < (mWidth / CUT_HALF + HITBOX_MARGIN_X) && abs((int)(missileBullet->mY - mY)) < mHeight / CUT_HALF + HITBOX_MARGIN_Y)
		{
			//弾の無効化
			missileBullet->mIsActive = false;
		    //無敵フラグ(Unbeatable)がfalseの場合はダメージ処理・スコア加算・を行う
			if (!mIsInvincible)
			{
				*score += (int)HIT_SCORE;
	...
	{
		//敵とスペシャル弾の位置を計算して弾が一定範囲内なら当たり判定を返す
		if (abs((int)(specialBullet->mX - mX)) < (mWidth / CUT_HALF + HITBOX_MARGIN_X) && abs((int)(specialBullet->mY - mY)) < mHeight / CUT_HALF + HITBOX_MARGIN_Y)
		{
			//弾の無効化
			specialBullet->mIsActive = false;
		    //無敵フラグ(Unbeatable)がfalseの場合はダメージ処理・スコア加算・を行う
			if (!mIsInvincible)
			{
				*score += (int)HIT_SCORE;
	...
	//ホーミング弾の判定
	if (mIsActive && homingBullet->mIsActive)
	{
		//敵とホーミング弾の位置を計算して弾が一定範囲内なら当たり判定を返す
		if (abs((int)(homingBullet->mX - mX)) < (mWidth / CUT_HALF + HITBOX_MARGIN_X) && abs((int)(homingBullet->mY - mY)) < mHeight / CUT_HALF + HITBOX_MARGIN_Y)
		{
			//弾の無効化
			homingBullet->mIsActive = false;
		    //無敵フラグ(Unbeatable)がfalseの場合はダメージ処理・スコア加算・を行う
			if (!mIsInvincible)
			{
				*score += (int)HIT_SCORE;
