...
@note	各ハンドル変数の初期化を行う。
@note	コンストラクタは private にしており、外部から直接インスタンス生成できないようにしている。
@note	インスタンスの生成は init 関数経由のみに制限しており、シングルトンとして動作させる設計。
@note	※初期化の書式については特に理由なく作成したため、他ファイルと統一されていない
*/
Data::Data()
{
	//---リザルトデータ---
...
@note	各ハンドル変数のリソース解放は Release 関数で行う設計のため、デストラクタ内では特に記載なし。
@note	デストラクタは private に設定し、外部からインスタンスの破棄ができないようにしている。
@note	インスタンスの生成・破棄を制御することで、シングルトンとして動作させている。
...
/*
@brief	インスタンスの生成を行うための関数。インスタンスが未生成の場合のみ生成する

@param     	なし
@return     なし
@note		インスタンスのポインタを private static にすることで、プログラム全体で1つのインスタンスを共有するようにしている
@note		コンストラクタが private のため、外部から直接インスタンスを生成できないように制限している
@note		public static 関数にすることで、オブジェクトを生成しなくても外部から呼び出せるようにしている
*/
...
/*
@brief	インスタンスを解放する関数。作成時解放処理をわすれており、使用されていない
...
@note		public static 関数のため、オブジェクトを生成しなくても外部から呼び出すことができる
@note		これにより、この関数を介してどこからでも同一のインスタンスにアクセスできる
*/
...
@param     	なし
@return     なし
@note     クラス内で保持している各ハンドルに対して、外部ファイルからデータを読み込み、使用可能な状態にする
...
void Data::load()
{
	...
	mPlayerBulletImageHandleArray[BULLET_NORMAL] = LoadGraph("Images/ballet.png");
	mPlayerBulletImageHandleArray[BULLET_MISSILE] = LoadGraph("Images/missaile.png");
	mPlayerBulletImageHandleArray[BULLET_SPECIAL] = LoadGraph("Images/specielBallet.png");
	...
	mEnemyMiniBulletImageHandleArray[MINI_BULLET_YELLOW] = LoadGraph("Images/enemyBalletmini.png");
	mEnemyMiniBulletImageHandleArray[MINI_BULLET_ORANGE] = LoadGraph("Images/enemyBalletmini2.png");
	...
	mUnbeatableItemGetSoundEffectHandle = LoadSoundMem("Sounds/se_powerup_009.mp3");
	...
	mEffectAnimationHandleArray[POWERUP_EF] = LoadGraph("Animations/Effect_p029.png");
	mEffectAnimationHandleArray[CHARGE_EF] = LoadGraph("Animations/pipo-btleffect107i.png");
	mEffectAnimationHandleArray[HIT_EF] = LoadGraph("Animations/花火透過.png");
	mEffectAnimationHandleArray[SHOT_EF] = LoadGraph("Animations/pipo-mapeffect009.png");
	mEffectAnimationHandleArray[LIFE_EF] = LoadGraph("Animations/pipo-btleffect110d.png");
	mEffectAnimationHandleArray[STAR_EF] = LoadGraph("Animations/pipo-btleffect110j.png");
	mEffectAnimationHandleArray[WARP_EF] = LoadGraph("Animations/warp_320.png");
	mEffectAnimationHandleArray[CHARGE_EF_BOSS] = LoadGraph("Animations/pipo-btleffect107i_2.png");
