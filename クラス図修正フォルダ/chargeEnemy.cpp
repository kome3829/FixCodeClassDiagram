    ...
    if (mIsActive)
    {
        ...
        if (!mIsBossStage)
        {
            if (mMoveCount % MOVE_INTERVAL_FRAME == 0)//２フレームに一回移動
            {
                ...
                else
                {
                    ...
                    if (mX >= SCREEN_CENTER_X)
                    {
                        mVectorY *= (ENEMY_SLOWDOWN_Y + CHARGE_ENEMY_SPD_MARGIN);//徐々に減速
                        mY += mVectorY;
                        mX -= mVectorX;
                    }
                    else
                    {
                        mVectorY *= (ENEMY_SPEEDUP_Y - CHARGE_ENEMY_SPD_MARGIN);//徐々に加速
                        mY -= mVectorY;
                        mX -= mVectorX;
                    }
                }
        ...
        else
        {
            if (mNumber == SUMMON_LEFT)//左側
            {
                ...
                if (abs((int)mX - CHARGE_ENEMY_SUMMON_POINT_LEFT_X) >= ENEMY_POSITION_DIFFERENCE_RANGE &&
                    abs((int)mY - CHARGE_ENEMY_SUMMON_POINT_LEFT_Y) >= ENEMY_POSITION_DIFFERENCE_RANGE &&
                ...
                if (mMoveCount > ARC_MOVE_START_FRAME)
                {
                    ...
                    mX = LEFT_CIRCLE_CENTER_POSITION_X + cos(mArcMotionAngle) * mArcMotionRadius;
                    mY = LEFT_CIRCLE_CENTER_POSITION_Y + sin(mArcMotionAngle) * mArcMotionRadius;
                }
            }
            if (mNumber == SUMMON_RIGHT)//右側
            {
				//位置についたか判定
				//現在の位置と指定位置のずれが一定値になるまで処理を実行する
                if (abs((int)mX - CHARGE_ENEMY_SUMMON_POINT_RIGHT_X) >= ENEMY_POSITION_DIFFERENCE_RANGE &&
                    abs((int)mY - CHARGE_ENEMY_SUMMON_POINT_RIGHT_Y) >= ENEMY_POSITION_DIFFERENCE_RANGE &&
                    !mIsReachedBossPosition)
                {
					//atan2を使用して敵と指定位置とのなす角を計算
                ...
                if (mMoveCount > ARC_MOVE_START_FRAME)
                {
                    ...
                    mX = RIGHT_CIRCLE_CENTER_POSITION_X + cos(mArcMotionAngle) * mArcMotionRadius;
                    mY = RIGHT_CIRCLE_CENTER_POSITION_Y + sin(mArcMotionAngle) * mArcMotionRadius;
                }
            }
			//画面外判定
            if (mX >= MAX_SCREEN_WIDTH + mWidth || mX <= 0 - mWidth || mY >= MAX_SCREEN_HEIGHT + mHeight || mY <= 0 - mHeight)
			...
			//チャージ後の攻撃終了時まで無敵
            if (mMoveCount > UNBEATABLE_START_FRAME && mMoveCount <= UNBEATABLE_END_FRAME)
            {
                mIsInvincible = true;
            }
            //ワープ移動(280～330フレームの間)
            ...
            if (mMoveCount == ARC_MOVE_START_FRAME)
            {
                if (mNumber == SUMMON_LEFT)//左側
                {
				    ...
				    //円運動計算に必要な半径、角度を計算。
                    double vectorToCenterX = CHARGE_ENEMY_WARP_POINT_LEFT_X - LEFT_CIRCLE_CENTER_POSITION_X;
                    double vectorToCenterY = CHARGE_ENEMY_WARP_POINT_LEFT_Y - LEFT_CIRCLE_CENTER_POSITION_Y;

                    mArcMotionRadius = sqrt(vectorToCenterX * vectorToCenterX + vectorToCenterY * vectorToCenterY);
                ...
                else if (mNumber == SUMMON_RIGHT)//右側
                {
				    ...
				    //円運動計算に必要な半径、角度を計算。
                    double vectorToCenterX = CHARGE_ENEMY_WARP_POINT_RIGHT_X - RIGHT_CIRCLE_CENTER_POSITION_X;
                    double vectorToCenterY = CHARGE_ENEMY_WARP_POINT_RIGHT_Y - RIGHT_CIRCLE_CENTER_POSITION_Y;
...
void ChargeEnemy::draw()
{
	if (mIsActive)
	{
		...
		{
			//通常画像の表示
			DrawGraph((int)mX - mWidth / CUT_HALF, (int)mY - mHeight / CUT_HALF, Data::getInstance()->mChargeEnemyImageHandle, TRUE);
		}
	}
}
