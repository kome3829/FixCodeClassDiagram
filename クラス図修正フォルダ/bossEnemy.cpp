#include "BossEnemy.h"
#include "BulletManager.h" //互いにヘッダファイルで読込を行うのを避けるため
...
@note	HPのパーセント計算を行う
@note
ボス敵の行動フラグ(mIsActive)がtureで撃破フラグ(mIsDefeat)がfalseの場合に移動、弾発射、ダメージの処理を実行する
@note	行動を体力応じて分岐している
...
@note	ボス敵の行動フラグ(mIsActive)がtrueの場合のみ処理を実行する
...
/*
@brief	動作開始するときに必要なパラメータの初期設定や処理を行う関数

@param[in]	なし
@return		なし

@note	位置やフラグなどの初期値の設定が必要なパラメータの設定を行う
*/
...
bool BossEnemy::moveCenterPosition()
{
	...
	// 角度から速度（mVectorX,mVectorY）を計算し設定している
...
@note   MoveCount変数でカウントし、処理を管理
@note	moveCenterPosition関数を使い次の行動の初期位置へ移動
@note
...
void BossEnemy::takeDamage(int *score)
{
	...
	if (!mIsUnbeatable && !mIsDamageCoolDown)// ダメ処理インタバール中もしくは無敵でない場合ダメージ処理を実行
	{
	...
	// HPが０以下の場合は撃破フラグ(mIsDefeat)をtrueにする
	if (mHitPoint <= 0)
	{
		mIsDefeat = true;
	}
...
-
ボスのフラグ(mIsActive)がtrueで撃破フラグ(mIsDefeat)がfalseの場合のみ、処理を実行する
-
mShotMiniBulletCount変数及びmShotBulletCount変数でカウントし、処理を管理している
- 発射時に発射SEの再生をしている
